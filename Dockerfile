...
CMD /opt/kafka/bin/kafka-mirror-maker.sh --consumer.config /etc/kafka/consumer.props --producer.config /etc/kafka/producer.props --whitelist="${WHITELIST}" --abort.on.send.failure true --num.streams $STREAMS $KAFKA_MIRROR_MAKER_OPT
